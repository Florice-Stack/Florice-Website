        ...
        <div className="mx-auto grid w-full max-w-2xl gap-3 sm:grid-cols-2 sm:items-stretch">
          {areas.map(({ icon: Icon, title, href, description }, index) => (
            <Reveal key={href} delay={index * 70} className="h-full">
              <Link
                href={href}
                className="group flex h-full items-start gap-3 glass-panel rounded-md p-4 motion-safe:transition-all motion-safe:duration-200 motion-safe:hover:-translate-y-1 motion-safe:hover:border-olive/30 motion-safe:hover:shadow-lift"
              >
